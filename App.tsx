...
import { CheckCircle, AlertTriangle, Plus, Activity, History, Server, WifiOff, RefreshCw } from 'lucide-react';
...
function App() {
  ...
  return (
    <div className="min-h-screen font-sans pb-20 relative overflow-x-hidden">
      ...
      {/* Navbar */}
      <nav className="w-full border-b border-white/5 bg-[#020617]/80 backdrop-blur-md sticky top-0 z-40">
        <div className="max-w-5xl mx-auto px-6 h-16 flex items-center justify-between">
          <div className="flex items-center gap-3">
            <div className="relative group">
               <div className="absolute inset-0 bg-emerald-500 blur opacity-40 rounded-full group-hover:opacity-60 transition-opacity" />
               <Server className="relative text-emerald-400 w-5 h-5" />
            </div>
          ...
          <div className="flex items-center gap-4">
            <button 
               ...
               className="flex items-center gap-2 px-4 py-1.5 rounded-lg bg-emerald-500/10 hover:bg-emerald-500/20 border border-emerald-500/20 text-sm font-medium transition-all text-emerald-400 hover:text-emerald-300"
            >
              <Plus size={16} />
      ...
      <main className="max-w-4xl mx-auto px-4 sm:px-6 pt-12">
        ...
        <div className="text-center py-12 relative">
           {/* Background glow */}
           <div className={`absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-64 h-64 blur-[100px] rounded-full pointer-events-none transition-colors duration-700 ${allSystemsNormal ? 'bg-emerald-500/10' : 'bg-red-500/10'}`} />
          ...
          >
            <div className={`inline-flex items-center justify-center p-4 rounded-full mb-6 border shadow-[0_0_30px_rgba(0,0,0,0.2)] backdrop-blur-md transition-colors duration-500 ${
                allSystemsNormal 
                ? 'bg-emerald-500/5 border-emerald-500/20 shadow-emerald-500/10' 
                : 'bg-red-500/5 border-red-500/20 shadow-red-500/10'
            }`}>
              {loading && !statusData ? (
                  <RefreshCw className="w-10 h-10 text-zinc-400 animate-spin" strokeWidth={1.5} />
              ) : allSystemsNormal ? (
            ...
            <h1 className="text-4xl sm:text-5xl font-bold text-white tracking-tight mb-4 drop-shadow-xl">
            ...
            <div className="flex items-center justify-center gap-3 text-sm">
              ...
              {isUsingMock && (
                <span className="flex items-center gap-1.5 text-amber-400 bg-amber-400/10 px-2 py-0.5 rounded border border-amber-400/20 text-xs font-medium">
                  <WifiOff size={12} /> Demo Mode
                </span>
              )}
              
              {connectionError && !isUsingMock && (
                  <span className="text-red-400 text-xs bg-red-400/10 px-2 py-0.5 rounded border border-red-400/20">
                      Backend Unreachable
                  </span>
              )}
            </div>
        ...
        <div className="mb-16 relative z-10">
           ...
           {services.length === 0 && (
             ...
             >
               ...
               <p className="text-zinc-500 text-lg">No services are being monitored.</p>
               <button onClick={() => setIsModalOpen(true)} className="text-emerald-400 hover:text-emerald-300 font-medium mt-2 transition-colors">
        ...
        {services.length > 0 && (
            ...
            <div className="relative pl-8 border-l border-white/10 space-y-10">
                ...
                <div className="absolute -left-[37px] top-1 h-5 w-5 rounded-full border-4 border-[#020617] bg-emerald-500/50 shadow-[0_0_10px_rgba(16,185,129,0.5)]" />
      ...
      <footer className="text-center py-12 text-zinc-700 text-xs tracking-widest uppercase">
        <p>Secure Status Engine &bull; {isUsingMock ? 'Local Environment' : 'Connected to Cloud'}</p>
      </footer>
    </div>
  );
}
